...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        output_layer = tf.layers.dense(layer, output_size, activation=output_activation, )
        return output_layer
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            sy_logstd = tf.get_variable("std", shape=[self.ac_dim], dtype=tf.float32)
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            for reward in re_n:
                ...
                q = np.ones(shape = [len(reward)]) * ret_tau
                q_n.extend(q)
        return q_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            self.sess.run(self.baseline_update_op, feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: q_n})
            #target_n = None
        ...
        _, after_loss = self.sess.run([self.update_op, self.loss], feed_dict = {self.sy_ob_no : ob_no, self.sy_ac_na : ac_na, self.sy_adv_n : adv_n})
